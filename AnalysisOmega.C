...
void AnalysisOmega(Int_t numExps=10, Int_t numEvts=770, Bool_t doFit=kTRUE, Bool_t doFitBeta=kTRUE, Int_t seed=1)
{
  ...
  for (Int_t iExp=0;iExp<numExps;iExp++) {
    ...
    pdf2->SetParameter(0,1.); // Normalización de la probabilidad a 1
    /*
    ...
    */
    ...
    //
    if (!doFitBeta) {
      pdf1->FixParameter(1,0); // Fija beta a 0 (desintegración que conserva la paridad)
    ...
    { auto legend = new TLegend(0.36, 0.70, 0.64, 0.85);

    legend->AddEntry(pdf1, "Espín #frac{1}{2}", "l");
    legend->SetLineColor(kBlack);
    legend->AddEntry(pdf2, "Espín #frac{3}{2}");
    legend->AddEntry(pdf3, "Espín #frac{5}{2}", "l");
